...
export const MenuManagementModal: React.FC<MenuManagementModalProps> = ({ isOpen, onClose, date, onSave }) => {
  ...
  useEffect(() => {
    if (isOpen) {
      fetchDishes();
      fetchMenuForDate(format(date, 'yyyy-MM-dd'));
    }
  }, [isOpen, date]);
  ...
  const handleAddMenuItem = () => {
    if (!newMenuItem.trim() || menuItems.includes(newMenuItem.trim())) {
      toast.error(newMenuItem.trim() ? 'Questo piatto è già stato aggiunto.' : 'Seleziona un piatto.');
      return;
    }
        ...
        <Dialog.Panel className="w-full max-w-md transform rounded-2xl bg-white text-left align-middle shadow-xl transition-all flex flex-col" style={{ maxHeight: '90vh' }}>
            <div className="p-4 border-b border-gray-200">
                <div className="flex items-center justify-between">
                    ...
                    <button onClick={onClose} className="rounded-full p-1 hover:bg-gray-100"><X className="h-5 w-5 text-gray-500" /></button>
                </div>
            </div>
            <div className="flex-grow overflow-y-auto p-4 space-y-4">
                <h3 className="font-medium text-gray-900">{format(date, 'EEEE d MMMM yyyy', { locale: it })}</h3>
